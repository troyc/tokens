...
fn tokens() -> Command {
    Command::new(env!("CARGO_BIN_EXE_tokens"))
}
...
fn default_lists_files_and_omits_gitignored_target() {
    let dir = fixture();
    let text = stdout(tokens().arg(dir.path()));
...
fn all_mode_shows_comments_and_tests() {
    let dir = fixture();
    let text = stdout(tokens().arg("-a").arg(dir.path()));
...
fn lockfiles_flag_includes_cargo_lock() {
    let dir = fixture();
    let text = stdout(tokens().arg("-l").arg(dir.path()));
    assert!(text.contains("Cargo.lock"), "{text}");
    assert!(text.contains("src/lib.rs"), "{text}");
}
...
fn named_gitignored_file_is_still_counted() {
    ...
    let text = stdout(tokens().arg(&ignored));
