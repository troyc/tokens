...
fn generate_completion_bash() {
    let text = stdout(tc().args(["--generate-completion", "bash"]));
    assert!(text.contains("complete"), "{text}");
    assert!(text.contains("--all"), "{text}");
    assert!(text.contains("--lockfiles"), "{text}");
    assert!(
...
fn generate_completion_zsh() {
    let text = stdout(tc().args(["--generate-completion", "zsh"]));
    assert!(
        text.contains("#compdef") || text.contains("compdef"),
        "{text}"
    );
    assert!(text.contains("--all"), "{text}");
}
...
fn generate_completion_fish() {
    let text = stdout(tc().args(["--generate-completion", "fish"]));
    assert!(text.contains("complete -c tc"), "{text}");
    assert!(text.contains("-l all"), "{text}");
    assert!(text.contains("-l lockfiles"), "{text}");
}

#[test]
fn generate_completion_rejects_unknown_shell() {
    let output = tc()
        .args(["--generate-completion", "cmd"])
        .output()
        .unwrap();
